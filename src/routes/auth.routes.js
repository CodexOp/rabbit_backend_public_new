...
app.post('/login', async (req, res) => {
  try {
    ...
    let _user = await userCollection.findOne({ address });
    if (!_user) {
      const newUser = {
        address,
        betttingHistory: [],
      ...
      _user = await userCollection.insertOne(newUser);
    }

    const token = jwt.sign({ address, _id: _user._id || _user.insertedId }, secretKey, { expiresIn: '24h' });
