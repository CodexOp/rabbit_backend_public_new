...
app.post('/deposit', async (req, res) => {
  try {
    ...
    if(transaction.to !== process.env.CONTRACT){
      return res.status(409).send({ error: 'Deposit function not called' });
    }
    const iface = new ethers.utils.Interface(contractAbi);
    const transactionData = iface.parseTransaction({ data: transaction.data, value: transaction.value });
    if (transactionData.name !== 'a') {
      return res.status(409).send({ error: 'Deposit function not called' });
    }
    const amount = parseFloat(ethers.utils.formatUnits(transactionData.args._amount, 18));
    console.log(amount, transactionData);
...
app.post('/withdraw', async (req, res) => {
  try {
    ...
    if (amount <= 1000) {
      return res.status(400).send({ error: 'Amount must be greater than 1000' });
    }
    ...
    const contract = new ethers.Contract(process.env.CONTRACT_ADDRESS, ['function b (uint256 _amount) external returns (bool)'], account);
    const tx = contract.b(ethers.utils.parseEther(amount.toString()));
