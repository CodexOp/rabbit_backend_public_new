...
app.get('/user/:address', async (req, res) => {
  try {
    ...
    const data = await userCollection.findOne({ address });
    res.status(200).send({ message: 'Success', data });
  } catch (error) {
    console.error('Error fetching gameInfo', error);
    return res.status(500).send({ error: 'Internal Server Error' });
...
app.get('/totalUserBet/:address', async (req, res) => {
  try {
    const { address } = req.params;
    const data = await userCollection.findOne({ address });
    let totalAmount = 0;
    for ( let i=0; i< data.txnHistory.length; i++){
      if (data.txnHistory[i].type === 'bet')
      totalAmount += data.txnHistory[i].amount;
    }

    res.status(200).send({ message: 'Success', totalAmount });
  } catch (error) {
    console.error('Error fetching gameInfo', error);
    return res.status(500).send({ error: 'Internal Server Error' });
  }
